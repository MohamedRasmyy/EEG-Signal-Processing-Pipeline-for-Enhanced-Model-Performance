...
def Preprocessing(l_freq_eog, h_freq_eog, l_freq_eeg, h_freq_eeg):
    ...
    for file_path in file_paths:
        ...
        # 6. Extract Events and Epoch the Data
        events, event_id = mne.events_from_annotations(raw)
        epochs = mne.Epochs(raw, events, event_id, tmin=-0.5, tmax=4.0, baseline=(-0.5, 0), preload=True, event_repeated='merge')
        epochs.plot(theme='light')
        
        # 7. EEG and EOG picks selecction
        ...
        ica_eeg.fit(epochs, picks=picks)
        
        # Find and exclude bad components related to EOG artifacts
        eog_indices, scores = ica_eeg.find_bads_eog(epochs, ch_name=eog_channels)
        ica_eeg.plot_scores(scores)
        ica_eeg.plot_sources(epochs, show_scrollbars=False)
        ...
        # 8. ASR for Noise Removal For the most noisy subjects"A04T,A09T"
        if file_path=='A04T.gdf' or file_path == 'A09T.gdf':
        ...
        # Save cleaned data
        raw_clean_eeg.save('cleaned_raw-epo_ICA_ASR49.fif', overwrite=True)
        raw_clean_eeg.plot(title='Raw Data After ICA_ASR49')
        
        # 9. Plot Power Spectral Density (PSD) Before and After ICA*ASR
        fig, ax = plt.subplots(2, 1, figsize=(10, 6))
        raw.plot_psd(ax=ax[0], fmax=50, show=False, color='r')
        ax[0].set_title('PSD Before ICA')
        raw_clean_eeg.plot_psd(ax=ax[1], fmax=50, show=False, color='g')
        ax[1].set_title('PSD After ICA&ASR49')
        plt.tight_layout()
        plt.show()
        
        # 10. Epoch Cleaned Data (Events of Interest)
    ...
    # 12. Concatenate All Data and Labels Across Subjects
    X_all_ICA_ASR49 = np.concatenate(all_data, axis=0)
    Y_all_ICA_ASR49 = np.concatenate(all_labels, axis=0)
    
    #Save the preprocessed data
    np.save('X_all_ICA_ASR49.npy', X_all_ICA_ASR49)
    np.save('Y_all_ICA_ASR49.npy', Y_all_ICA_ASR49)
    

    # Return the concatenated data and labels
    return X_all_ICA_ASR49, Y_all_ICA_ASR49
