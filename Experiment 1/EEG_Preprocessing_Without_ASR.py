...
def Preprocessing(l_freq_eog, h_freq_eog, l_freq_eeg, h_freq_eeg):
    ...
    for file_path in file_paths:
        ...
        raw_clean_eeg.save('cleaned_raw-epo_Without_ASR.fif', overwrite=True)
        raw_clean_eeg.plot(title='Raw Data After ICA')
    ...
    X_all_No_ASR = np.concatenate(all_data, axis=0)
    Y_all_NO_ASR = np.concatenate(all_labels, axis=0)
    
    #Save the preprocessed data
    np.save('X_all_No_ASR.npy', X_all_No_ASR)
    np.save('Y_all_No_ASR.npy', Y_all_NO_ASR)
    
    # Return the concatenated data and labels
    return X_all_No_ASR, Y_all_NO_ASR
